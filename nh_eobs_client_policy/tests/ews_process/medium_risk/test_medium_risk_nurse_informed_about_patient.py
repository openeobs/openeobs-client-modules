...
class TestMediumRiskNurseInformedAboutPatient(TransactionCase):
    """
    ...
    def setUp(self):
        super(TestMediumRiskNurseInformedAboutPatient, self).setUp()
        ...
        self.data_model = 'nh.clinical.notification.nurse'
        self.act_summary = 'Informed About Patient'
    ...
    def test_hca_task_created(self):
        ...
        activity = self.test_utils.get_open_activities_for_patient(
            data_model='nh.clinical.notification.hca',
            user_id=self.hca.id
        )
    ...
    def test_nurse_task_after_pme(self):
        """
        Test that the Nurse's task is no longer available after PME
        """
        self.check_number_of_activities(1)
    ...
    def test_nurse_task_after_transfer_out(self):
        """
    ...
    def test_nurse_task_after_transfer_in(self):
        """
    ...
    def test_nurse_task_after_partial(self):
        """
        Test that after a partial observation the Nurse should still see
        the Informed About Patient task
        """
        self.check_number_of_activities(1)
    ...
    def test_nurse_task_on_workload_view(self):
        """
        Test that Informed About Patient task is visible in the workload view
        on the desktop
        """
    ...
    def test_nurse_task_on_overdue_tasks_view(self):
        """
        Test that the Informed About Patient task is in the overdue tasks view
        when it's overdue
        """
    ...
    def test_nurse_task_on_report_in_progress(self):
        """
        Test that the Informed About Patient task is in the report for the
    ...
    def test_nurse_task_on_report_completed(self):
        """
        Test that the Informed About Patient task is in the report for the
        patient when it's been completed
        """
        self.test_utils.complete_open_activity(self.data_model)
        task_status = self.test_utils.get_report_triggered_action_status(
            self.act_summary)
        self.assertTrue('Date: ' in task_status)

    def test_nurse_task_after_shift_change(self):
        """
        Test that the Informed About Patient task is in task list of new nurses
        when a shift change happens
        """
        # TODO: Eckhard to clarify correct behaviour
